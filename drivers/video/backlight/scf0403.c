...
static int __devinit scf0403_probe(struct spi_device *spi)
{
	...
	ret = scf0403_spi_transfer(spi, disp_ids, NULL, 0, ids_buff, 3);
	if (ret < 0) {
		dev_err(&spi->dev, "SPI transfer failure while reading IDs: %d \n", ret);
		goto free_gpio;
	}
	if (((ids_buff[0] == 0x00) && (ids_buff[1] == 0x00) && (ids_buff[2] == 0x00)) ||
	    ((ids_buff[0] == 0xff) && (ids_buff[1] == 0xff) && (ids_buff[2] == 0xff))) {

		dev_err(&spi->dev, "Invalid device IDs \n");
		ret = -ENODEV;
		goto free_gpio;
	}

	dev_info(&spi->dev, "Device IDs: 0x%02X %02X %02X\n",
